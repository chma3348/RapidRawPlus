...
export default function AppModals(props: AppModalsProps) {
  ...
  const showEnhancePill =
    !enhanceModalState.isOpen && (enhanceModalState.isProcessing || enhanceModalState.hasUnseenResult);
    ...
    <>
      {showEnhancePill && (
        <button
          ...
          className="fixed bottom-4 right-4 z-40 flex items-center gap-2 px-3 py-2 rounded-full shadow-lg bg-surface border border-border-color hover:bg-card-active transition-colors text-sm text-text-primary"
        >
          {enhanceModalState.isProcessing ? (
            <>
              <span className="w-3 h-3 rounded-full border-2 border-accent border-t-transparent animate-spin" />
              <span>{enhanceModalState.progressMessage || t('modals.enhance.backgroundRunning')}</span>
            </>
          ) : (
            <>
              <span className="w-2 h-2 rounded-full bg-accent animate-pulse" />
              <span>{t('modals.enhance.backgroundReady')}</span>
            </>
          )}
        </button>
      )}
