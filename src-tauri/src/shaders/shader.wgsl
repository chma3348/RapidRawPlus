...
// Resolve shadow-lift correction lives here because it samples a texture
// bound only to this shader; the lift itself is in tone_v2.wgsl.
...
fn apply_highlights_adjustment(
    ...
    if (adjustments.global.process_version >= 2u) {
        return apply_highlights_adjustment_v2(color_in, blurred_color_input_space, neighborhood_input_space, is_raw, highlights_adj);
    }
...
fn agx_full_transform(color_in: vec3<f32>) -> vec3<f32> {
    return agx_full_transform_with(
        color_in,
        adjustments.global.agx_pipe_to_rendering_matrix,
        adjustments.global.agx_rendering_to_pipe_matrix,
    );
}
