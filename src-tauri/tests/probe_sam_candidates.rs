...
fn dump_candidates() {
    ...
    if let Ok(ex) = std::env::var("SUBJECT_EXCLUDE") {
        ...
        coords.extend([(e[0] * scale) as f32, (e[1] * scale) as f32]);
        labels.push(0.0);
    }
    ...
    let out = session
        .run(ort::inputs![
            ...
            Tensor::from_array(
                Array::from_shape_vec((1, 1, 256, 256), vec![0.0f32; 65536]).unwrap()
            )
            .unwrap(),
    ...
    let sx = (w as f64 * scale) as usize;
    let sy = (h as f64 * scale) as usize; // valid region in 1024 space
    let vw = (sx * side / 1024).max(1);
    let vh = (sy * side / 1024).max(1);
    let outdir = std::env::var("SUBJECT_OUTPUT").ok().map(PathBuf::from);
    if let Some(d) = &outdir {
        std::fs::create_dir_all(d).unwrap();
    }
    println!(
        "valid grid {vw}x{vh} of {side}; click grid ({:.1},{:.1})",
        click[0] * scale * side as f64 / 1024.0,
        click[1] * scale * side as f64 / 1024.0
    );
    for (i, m) in data.chunks_exact(area).enumerate() {
        ...
        println!(
            ...
            scores[i],
            pos,
            100.0 * pos as f32 / (vw * vh) as f32,
            stable as f32 / possible as f32,
            m[cy * side + cx]
        );
        if let Some(d) = &outdir {
