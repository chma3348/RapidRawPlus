...
fn sky_patch_round_trip() {
    ...
    for (name, options) in [
        (
            "auto match (whole frame)",
            sky_replace::SkyReplaceOptions::auto_match(),
        ),
        (
            "as shot (sky only)",
            sky_replace::SkyReplaceOptions::as_shot(),
        ),
    ] {
        let base = photo.to_rgb8();
        let t = std::time::Instant::now();
        let result = sky_replace::replace_sky(&photo, &sky.mask, &plate, &options)
            .unwrap()
            .to_rgb8();
        ...
        let gap = |pick: &dyn Fn(u32, u32) -> [f32; 3]| {
            ...
            (
                sum / (result.pixels().len() * 3) as f64 * 255.0,
                worst * 255.0,
            )
        };
        let (legacy_mean, legacy_worst) =
            gap(&|x, y| legacy.get_pixel(x, y).0.map(|v| v as f32 / 255.0));
        let (v3_mean, v3_worst) = gap(&|x, y| {
            frame.pixels.get_pixel(x, y).0[..3]
                .try_into()
                .map(|a: [f32; 3]| a.map(encode_srgb))
                .unwrap()
        });
        println!(
        ...
        assert!(
            legacy_mean < 1.5 && v3_mean < 1.5,
            "the patch does not reproduce the composite"
        );
    }
}
