...
fn setup_models_dir() -> tempfile::TempDir {
    ...
    fs::write(
        manifests_dir.join("ghost_model.json"),
        MISSING_WEIGHTS_MANIFEST,
    )
    .unwrap();
...
fn dummy_model_round_trip() {
    ...
    let dummy = test_models
        .iter()
        .find(|m| m.id == "dummy-identity")
        .unwrap();
    ...
    assert!(
        masks
            .iter()
            .any(|m| m.id == "sam-vit-b" && m.builtin && !m.available)
    );
    ...
    let input =
        Array::from_shape_vec(IxDyn(&[1, 3, 4, 4]), (0..48).map(|v| v as f32).collect()).unwrap();
    ...
    assert!(
        err.to_string().contains("weight file is missing"),
...
fn tiled_enhancement_fixed_size_padding() {
    ...
    let output = run_tiled_enhancement(&input, &session, 1, 512, 8, Some((128, 128)), |_, _| {})
...
fn bundled_catalog_is_valid() {
    ...
    for entry in &catalog {
        let dl = entry.manifest.download.as_ref().unwrap_or_else(|| {
            panic!("catalog entry '{}' has no download spec", entry.manifest.id)
        });
        assert_eq!(
            dl.sha256.len(),
            64,
...
fn expansion_canvas_and_mask_geometry() {
    ...
    assert_eq!(
        mask.get_pixel(5, 5)[0],
        255,
        "new left strip must be masked"
    );
    assert_eq!(
        mask.get_pixel(150 - 1, 109)[0],
        255,
        "new bottom strip must be masked"
    );
    assert_eq!(
        mask.get_pixel(100, 40)[0],
        0,
        "deep inside original must be unmasked"
    );
    assert_eq!(
        mask.get_pixel(32, 40)[0],
        255,
        "seam band inside expanded edge must be masked"
    );
    // Right edge was not expanded: no seam band there.
    assert_eq!(
        mask.get_pixel(149 - 1, 40)[0],
        0,
        "unexpanded right edge must stay unmasked"
    );
}
