...
fn replace() {
    ...
    let scene = registry
        ...
        .expect("scene model");
    let out = PathBuf::from(env("SKY_OUT"));
    std::fs::create_dir_all(&out).unwrap();
    // SKY_VARIANTS: name=json,name=json — each is composited and saved.
    let variants: Vec<(String, sky_replace::SkyReplaceOptions)> =
        match std::env::var("SKY_VARIANTS") {
            Ok(v) => v
                .split(';')
                .filter(|s| !s.is_empty())
                .map(|spec| {
            ...
            Err(_) => vec![(
                "default".to_string(),
                sky_replace::SkyReplaceOptions::default(),
            )],
        };
    ...
    for photo_path in env("SKY_PHOTOS").split(',') {
        ...
        let Some(sky) =
            scene_masks::sky_mask_scene(&photo, &scene, scene_masks::Orientation::default())
                .unwrap()
        else {
        ...
        let name = PathBuf::from(photo_path)
            .file_stem()
            .unwrap()
            .to_string_lossy()
            .to_string();
        for plate_path in &plates {
            ...
            for (variant, opts) in &variants {
                ...
                done.save(out.join(format!("{name}__{stem}__{variant}.jpg")))
                    .unwrap();
                println!(
                    "{name} + {stem} [{variant}]: sky {:.0}% [mask {t_mask:.1?}, composite {:.1?}]",
                    sky.coverage * 100.0,
                    t.elapsed()
                );
            }
